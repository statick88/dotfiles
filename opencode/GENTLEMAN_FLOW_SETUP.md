...
### 3. Skills SDD Instalados (10 skills)
...
| **sdd-prd** | Generar PRD funcional y de negocio | `opencode sdd-prd` |
| **sdd-spec** | Escribir especificaciones técnicas desde PRD | `opencode sdd-spec` |
...
> sdd-prd
```
Define objetivos de negocio, alcance funcional, KPIs y criterios de aceptación.

Checklist mínimo del PRD (basado en Gentleman Programming):
- Objetivo de negocio y problema a resolver
- Alcance (in-scope / out-of-scope)
- User stories y criterios de aceptación con IDs trazables (`AC-###`)
- **Personas** (del libro):
  - Persona primaria con nivel de experiencia (Junior/Senior/Specialist)
  - Al menos 2 personas secundarias (Tech Lead, PO, Architect, Specialists, AI Assistant si aplica)
- **Stakeholders y aprobadores**:
  - Product Owner + Engineering Owner (obligatorios)
  - Security Auditor (OWASP, JWT, secretos) cuando hay riesgo de seguridad
  - Accessibility Auditor (WCAG 2.1 AA) cuando hay cambios de UI
- **KPIs con baseline + target + ventana** (al menos 1 por categoría):
  - Negocio: conversión, adopción, revenue
  - UX: Core Web Vitals (LCP < 2.5s, FID < 100ms, CLS < 0.1), Psicología de la Espera (< 100ms instantáneo)
  - Calidad: cobertura de tests (100% funciones, 80% líneas/ramas/statements), ritmo sostenible
  - IA (si aplica): tiempo de respuesta, tasa de alucinación, costo por token/tarea
- **Rituales** (owner + cadencia):
  - Sprint Planning (1-2 semanas): feature → story → task
  - Daily Meeting: sincronización
  - Mid-cycle Review: detección temprana de riesgos
  - Retrospectiva: mejora continua
  - Pair Programming (SHOULD): compartir contexto
  - Ventanas de refactoring (SHOULD): inicio/fin de sprint
  - Auditoría de Seguridad y Accesibilidad: obligatoria antes de merge a main
- **Quality Gates (Definition of Done)**:
  - Criterios de aceptación cumplidos, tests pasando, código refactorizado y con lint
  - Workflow Tony Stark: Define → Orchestrate → Execute → Review → Iterate
  - Reglas inquebrantables: no código sin funcionalidad concreta, no implementar sin tests fallando (RED)

### Paso 4: Explorar y Diseñar
```
> sdd-explore
```
...
> sdd-design
```
Crea diseño técnico alineado al PRD.

### Paso 5: Especificaciones
```
> sdd-spec
```
Traduce el PRD a requisitos técnicos verificables (Given/When/Then).

Regla de trazabilidad PRD → Spec:
- Cada criterio de aceptación del PRD debe mapear a al menos 1 Requirement en `spec.md`
- Cada Requirement debe tener escenarios (happy path + edge case)
- `sdd-verify` falla si hay requirements sin origen en PRD

### Paso 6: Desglosar Tareas
```
> sdd-tasks
```
...
> sdd-verify
```
Valida contra PRD + SPEC.md.

Reglas de validación en `sdd-verify` (alineadas con Gentleman Programming):
- **BLOCKED**: si falta `prd.md` → no avanza
- **CRITICAL**: requirement sin traza a `AC-###` del PRD
- **CRITICAL**: faltan aprobaciones obligatorias (PO, Engineering Owner)
- **CRITICAL**: falta auditoría de seguridad/accesibilidad en cambios de producción
- **WARNING**: personas insuficientes (< 2 secundarias) o sin nivel de experiencia
- **WARNING**: KPIs sin baseline/target/ventana de medición
- **WARNING**: sin evidencia de rituales (planning, daily, review, retro)
- **WARNING**: Definition of Done incompleto (tests, refactoring, lint)
- **SUGGESTION**: sin evidencia de pair programming o ventanas de refactoring
- **SUGGESTION**: sin evidencia de ritmo sostenible (sustainable pace)

### Paso 9: Archivar
```
> sdd-archive
```
Sincroniza PRD + spec delta con spec principal y archiva.
...
│        ↓                                                        │
│  3. sdd-spec (especificaciones técnicas)                       │
│        ↓                                                        │
│  4. sdd-design (diseño técnico)                                │
│        ↓                                                        │
│  5. sdd-tasks (checklist de tareas)                            │
│        ↓                                                        │
│  6. sdd-apply (implementar tareas)                             │
│        ↓                                                        │
│  7. git add . && git commit                                    │
│        ↓                                                        │
│  8. 🤖 GGA revisa el código → PASS/FAIL                        │
│        ↓                                                        │
│  9. sdd-verify (validar contra PRD+spec)                       │
│        ↓                                                        │
│  10. git push                                                   │
│        ↓                                                        │
│  11. CI/CD + PR Review (GGA --pr-mode)                         │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
### Skills de OpenCode Disponibles (23 skills)

Gentleman.Dots incluye estos skills en `GentlemanOpenCode/skill/`:

| Skill | Descripción |
|-------|-------------|
| **sdd-*** (10) | Spec-Driven Development: init, propose, prd, spec, explore, design, tasks, apply, verify, archive |
...
# 5. Explorar código existente (Engram busca contexto)
> sdd-explore
...
# 9. Commit (GGA revisa automáticamente)
git add .
...
# 12. Push (GGA en CI revisa el PR completo)
git push
