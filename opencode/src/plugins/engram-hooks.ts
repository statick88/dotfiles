...
export class HookManager {
   ...
   */
  async onSessionCompacting(context: CompactingContext): Promise<void> {
    try {
  ...
  registerHooks(): Record<string, Function> {
    return {
      'system.transform': (ctx: SystemContext) => this.transformSystemPrompt(ctx),
      'session.compacting': (ctx: CompactingContext) => this.onSessionCompacting(ctx),
    };
  }
}
