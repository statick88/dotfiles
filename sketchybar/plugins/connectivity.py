...
def populate_wifi_menu(name):
    ...
    for i, ssid in enumerate(networks):
        ...
        # Properly quote the SSID to handle special characters
        quoted_ssid = ssid.replace("'", "'\\''")
        # Add error handling to the click script
        click_script = f"""
            if networksetup -setairportnetwork en0 '{quoted_ssid}'; then
                echo "Connected to {quoted_ssid}"
            else
                echo "Failed to connect to {quoted_ssid}"
                # Optional: Show an alert
                # osascript -e 'display alert "WiFi Connection Failed" message "Could not connect to {quoted_ssid}"'
            fi
            sketchybar --set {name} popup.drawing=off
            # Refresh the WiFi status after connection attempt
...
def main():
    ...
    sbar_set(
        name, {"label": "", "label.drawing": "off", "icon": icon, "icon.color": color}
    )
