...
def main():
    ...
    if charging:
        if percentage == 100:
            ...
            if state["last_alert"] != "full":
                notify(
                    "✅ Carga Completa",
                    "Desconecta el cargador para proteger la vida útil 🔋",
                )
        else:
        ...
        # Discharge states
        if percentage <= 5:
            current_alert = "emergency"
            if state["last_alert"] != "emergency":
                notify(
                    "🚨 EMERGENCIA: Batería al 5%",
                    "¡Apaga apps innecesarias! Conecta el cargador AHORA ⚡",
                )
        elif percentage <= 10:
            current_alert = "critical"
            if state["last_alert"] != "critical" and state["last_alert"] != "emergency":
                notify(
                    "🚨 CRÍTICO: Batería al 10%",
                    "¡Último aviso! Conecta el cargador inmediatamente 🔌⚡",
                )
        elif percentage <= 20:
            current_alert = "low"
            if (
                state["last_alert"] != "low"
                and state["last_alert"] != "critical"
                and state["last_alert"] != "emergency"
            ):
                notify(
                    "⚠️ Peligro Batería",
                    "Busca tu cargador y una fuente de electricidad ⚡",
                )
    ...
    sbar_set(name, {"label": f"{percentage}%", "icon": icon, "icon.color": color})
